-- ============================================================================
-- Verifactu: numero de instalacion SIF por empresa
-- ============================================================================
-- El NumeroInstalacion se solicita al servicio del productor para la version
-- actual del programa, la razon social, el NIF del obligado y el SIF FZ.
-- Idempotente: anade las columnas si faltan y recrea vi_empresas.
-- ============================================================================
DELETE FROM fza_usuarios_perfiles
 WHERE KEY_USUPER = 'frmMtoAppParam'
   AND SUBKEY_USUPER = 'appVerifactuIdInstalacion';
SET @sExisteCol := (
  SELECT COUNT(*)
...
SET @sSql := IF(@sExisteCol = 0,
  'ALTER TABLE fza_empresas
     ADD COLUMN NUMERO_INSTALACION_EMP varchar(60) NULL DEFAULT NULL
     COMMENT ''NumeroInstalacion SIF devuelto por el servicio del productor''
     AFTER FECHA_HASTA_CERTIFICADO_EMP',
  'SELECT ''NUMERO_INSTALACION_EMP ya existe, se omite'' AS info'
);
...
SET @sSql := IF(@sExisteCol = 0,
  'ALTER TABLE fza_empresas
     ADD COLUMN VERSION_INSTALACION_EMP varchar(60) NULL DEFAULT NULL
     COMMENT ''Version de Factuzam usada al obtener NumeroInstalacion''
     AFTER NUMERO_INSTALACION_EMP',
  'SELECT ''VERSION_INSTALACION_EMP ya existe, se omite'' AS info'
);
PREPARE stmt FROM @sSql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sSql := IF(@sExisteCol = 0,
  'ALTER TABLE fza_empresas
     ADD COLUMN CODIGO_SIF_INSTALACION_EMP varchar(2) NULL DEFAULT NULL
     COMMENT ''IdSistemaInformatico usado al obtener NumeroInstalacion''
     AFTER VERSION_INSTALACION_EMP',
  'SELECT ''CODIGO_SIF_INSTALACION_EMP ya existe, se omite'' AS info'
);
PREPARE stmt FROM @sSql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sSql := IF(@sExisteCol = 0,
  'ALTER TABLE fza_empresas
     ADD COLUMN INSTANTE_INSTALACION_EMP datetime NULL DEFAULT NULL
     COMMENT ''Instante de obtencion de NumeroInstalacion''
     AFTER CODIGO_SIF_INSTALACION_EMP',
  'SELECT ''INSTANTE_INSTALACION_EMP ya existe, se omite'' AS info'
);
PREPARE stmt FROM @sSql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SELECT `fza_empresas`.`CODIGO_EMP_EMP`                AS `CODIGO_EMP_EMP`,
       ...
       `fza_empresas`.`ESIVA_RECARGO_COMPRAS_EMP`
              AS `ESIVA_RECARGO_COMPRAS_EMP`,
       ...
       `fza_empresas`.`NUMERO_INSTALACION_EMP`
              AS `NUMERO_INSTALACION_EMP`,
       `fza_empresas`.`VERSION_INSTALACION_EMP`
              AS `VERSION_INSTALACION_EMP`,
       `fza_empresas`.`CODIGO_SIF_INSTALACION_EMP`
              AS `CODIGO_SIF_INSTALACION_EMP`,
       `fza_empresas`.`INSTANTE_INSTALACION_EMP`
              AS `INSTANTE_INSTALACION_EMP`,
