...
$temporal = Join-Path $raizTemporal ('fzam_hojacalculo_fase2_' + $PID)
...
Anotar ('Pruebas HojaCalculo Fase 2 - ' +
...
if ($candidatos.Count -eq 0) {
  ...
  $dpr = Join-Path $PSScriptRoot 'PruebasHojaCalculoFase2.dpr'
  ...
  foreach ($plataforma in @('Win32', 'Win64')) {
    ...
    $orden = 'call "' + $rsvars + '" && ' + $compilador +
      ' -B -Q -U"' + $rutaLib + ';C:\DISCO_DURO\DUnitX\Source"' +
      ' -I"C:\DISCO_DURO\DUnitX\Source"' +
      ' -E"' + $salidaPlataforma + '"' +
      ' -N0"' + $salidaPlataforma + '"' +
      ' "' + $dpr + '"'
    ...
    if ($codigoCompilacion -ne 0) {
      ...
      $ejecutable = Join-Path $salidaPlataforma 'PruebasHojaCalculoFase2.exe'
