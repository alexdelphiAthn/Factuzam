...
$temporal = Join-Path $raizTemporal ('fzam_hojacalculo_fase0_' + $PID)
...
Anotar ('Pruebas HojaCalculo Fase 0 - ' +
...
if ($candidatos.Count -eq 0) {
  ...
  $dpr = Join-Path $PSScriptRoot 'PruebasHojaCalculoFase0.dpr'
  ...
  foreach ($plataforma in @('Win32', 'Win64')) {
    ...
    if ($codigoCompilacion -ne 0) {
      ...
      $ejecutable = Join-Path $salidaPlataforma 'PruebasHojaCalculoFase0.exe'
