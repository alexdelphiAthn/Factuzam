...
$temporal = Join-Path $raizTemporal ('fzam_hojacalculo_fase3_' + $PID)
...
Anotar ('Pruebas HojaCalculo Fase 3 - ' +
...
if ($candidatos.Count -eq 0) {
  ...
  $dpr = Join-Path $PSScriptRoot 'PruebasHojaCalculoFase3.dpr'
  ...
  foreach ($plataforma in @('Win32', 'Win64')) {
    ...
    if ($codigoCompilacion -ne 0) {
      ...
      $ejecutable = Join-Path $salidaPlataforma 'PruebasHojaCalculoFase3.exe'
