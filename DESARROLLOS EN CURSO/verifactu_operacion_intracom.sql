...
--                                (el destinatario autoliquida o esta exenta).
--
...
--
-- Idempotente: tabla con CREATE TABLE IF NOT EXISTS, semilla con INSERT IGNORE
-- (no pisa personalizaciones del usuario) y la columna con chequeo en
-- INFORMATION_SCHEMA.
-- =============================================================================
-- 1) Tabla catalogo -----------------------------------------------------------
...
INSERT IGNORE INTO fza_verifactu_operaciones
  (CODIGO_VFO, DESCRIPCION_VFO, AYUDA_VFO, CLAVE_REGIMEN_VFO,
   CALIFICACION_VFO, OPERACION_EXENTA_VFO, ESREPERCUTE_IVA_VFO, ORDEN_VFO,
   ESACTIVO_VFO, INSTANTE_ALTA, USUARIO_ALTA)
VALUES
  ('INTERIOR', 'Interior - regimen general',
   'Venta nacional con IVA. Es el comportamiento por defecto.',
   '01', 'S1', NULL, 'S', 10, 'S', NOW(), 'SEED'),
