...
$temporal = Join-Path $raizTemporal ('fzam_hojacalculo_fase1_' + $PID)
...
Anotar ('Pruebas HojaCalculo Fase 1 - ' +
...
if ($candidatos.Count -eq 0) {
  ...
  $dpr = Join-Path $PSScriptRoot 'PruebasHojaCalculoFase1.dpr'
  ...
  foreach ($plataforma in @('Win32', 'Win64')) {
    ...
    if ($codigoCompilacion -ne 0) {
      ...
      $ejecutable = Join-Path $salidaPlataforma 'PruebasHojaCalculoFase1.exe'
