...
//  SInvalidStringBaseIndex = 'Índice de base de cadena no válido';
  ...
  // --- Nuevas constantes para TEncoding ---
  SInvalidStringBaseIndex = 'Índice de base de cadena (StringBaseIndex) no válido';

  // --- Constantes para System.Notification (Centro de Notificaciones) ---
  SUserRejectedAccess = 'No se puede realizar la operación del centro de notificaciones. El usuario rechazó el acceso a (%s)';
  SNotificationCenterCannotPerformOperation = 'No se puede realizar la operación del centro de notificaciones. Debe solicitar permiso utilizando el método (%s).';
  SNotificationCenterFeatureIsNotSupported = 'El centro de notificaciones no admite la característica (%s).';

  // --- Constantes para Android (Permisos de ejecución) ---
  {$IF Defined(ANDROID)}
  SCannotRequestPermissions = 'No se pueden solicitar permisos en un componente de aplicación que no sea una Actividad (Activity)';
  {$ENDIF}

  // --- Constantes de Color (Nuevas en versiones recientes) ---
  SInvalidColor = '''%s'' no es un valor de color %s válido';
