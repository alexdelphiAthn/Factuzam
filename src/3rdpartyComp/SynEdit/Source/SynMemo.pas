...
var
  ReplText: UnicodeString;
begin
  if ReadOnly then Exit;
  DoOnPaintTransient(ttBefore);

  // 1. Obtener el texto a reemplazar
  if IsWindowUnicode(Handle) then
    ReplText := PWideChar(Message.lParam)
  else
    ReplText := UnicodeString(PAnsiChar(Message.lParam));

  // 2. Si WParam es 0, significa que NO se debe poder deshacer (fCanUndo = False)
  if Message.WParam = 0 then
    LockUndo;

  try
    // 3. Al asignar SelText, SynEdit hace la magia del Undo automáticamente
    SelText := ReplText;
  finally
    // 4. Desbloqueamos el historial si lo habíamos bloqueado
    if Message.WParam = 0 then
      UnlockUndo;
  end;
