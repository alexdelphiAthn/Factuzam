...
Type
 ...
 TStreamLineReader = class(TLineReader)
  private
    FReader: TStreamReader; // Usa System.Classes
  public
  ...
  private
    ...
    FKeyWords        : TDictionary<string, TSQLToken>;
...
begin
  ...
  FKeyWords := TDictionary<string, TSQLToken>.Create;
end;
...
begin
  ...
  SetString(FCurTokenString, TokenStart, Len);
...
var
  ...
  Len: Integer;
  PrevToken: Char;
  TempStr: string; // Variable auxiliar
begin
  Inc(TokenStr);
  TokenStart := TokenStr;
  FCurTokenString := '';
  ...
  begin
    if (TokenStr[0] = #0) then
    begin
      // Fin de línea dentro del comentario
      Len := TokenStr - TokenStart + 1;
      
      // Extraemos lo que llevamos de línea
      if Len > 1 then
      begin
        SetString(TempStr, TokenStart, Len - 1);
        FCurTokenString := FCurTokenString + TempStr;
      end;
      
      // Agregamos el salto de línea manual
      FCurTokenString := FCurTokenString + #10;
  ...
  // Extraer la última parte antes del */
  Len := TokenStr - TokenStart - 1; // -1 para quitar el *
  if Len > 0 then
  begin
    SetString(TempStr, TokenStart, Len);
    FCurTokenString := FCurTokenString + TempStr;
  end;
...
var
  ...
  begin
    if Len > 0 then
    begin
      SetString(TempBuf, TokenStart, Len);
      FCurTokenString := FCurTokenString + TempBuf;
    end;
    if S <> '' then
...
begin
  ...
  Inc(TokenStr);
  TokenStart := TokenStr;
  FCurTokenString := ''; // Iniciamos vacío
  while not(TokenStr[0] = #0) do
  begin
    if (TokenStr[0] = Delim) then
    begin
...
//      Move(TokenStart^, FCurTokenString[1], Len);
    ...
    // Cambiar la búsqueda
    if FKeyWords.TryGetValue(string(S), Token) then
      Result := Token;
  end;
...
begin
  inherited Create;
  // Detecta automáticamente si es UTF8, ANSI o Unicode
  FReader := TStreamReader.Create(AStream, TEncoding.UTF8, True);
end;
...
begin
  // El scanner original espera un salto de línea al final para procesar comentarios
  if not FReader.EndOfStream then
    Result := FReader.ReadLine + #10
  else
    Result := '';
end;
