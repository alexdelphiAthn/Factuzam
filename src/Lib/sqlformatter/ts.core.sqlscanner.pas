...
begin
  ...
  begin
    if (TokenStr[0] = #0) then
    begin
      // Fin de l�nea dentro del comentario
      Len := TokenStr - TokenStart + 1;

      // Extraemos lo que llevamos de l�nea
      if Len > 1 then
      begin
      ...
      // Agregamos el salto de l�nea manual
  ...
  // Extraer la �ltima parte antes del */
...
begin
  ...
  Inc(TokenStr);
  TokenStart := TokenStr;
  FCurTokenString := ''; // Iniciamos vac�o
  while not(TokenStr[0] = #0) do
  begin
    if (TokenStr[0] = Delim) then
    begin
...
begin
  inherited Create;
  // Detecta autom�ticamente si es UTF8, ANSI o Unicode
...
begin
  // El scanner original espera un salto de l�nea al final para procesar comentarios
