...
const
  SQLDataTypeNames: array [TSQLDataType] of string = (
    '',                           // sdtDomain
    'SMALLINT',                   // sdtSmallInt
    'INTEGER',                    // sdtInteger
    'FLOAT',                      // sdtFloat
    'DOUBLE PRECISION',           // sdtDoublePrecision
    'DECIMAL',                    // sdtDecimal
    'NUMERIC',                    // sdtNumeric
    'DATE',                       // sdtDate
    'TIMESTAMP',                  // sdtDateTime
    'TIME',                       // sdtTime
    'CHAR',                       // sdtChar
    'VARCHAR',                    // sdtVarChar
    'NATIONAL CHARACTER',         // sdtNChar
    'NATIONAL CHARACTER VARYING', // sdtNVarChar
    'CSTRING',                    // sdtCstring
    'BLOB'                        // sdtBlob
  );

  ForeignKeyActionNames: array [TForeignKeyAction] of string = (
    '',            // fkaNone
    'NO ACTION',   // fkaNoAction
    'CASCADE',     // fkaCascade
    'SET DEFAULT', // fkaSetDefault
    'SET NULL'     // fkaSetNull
  );
...
begin
  if DataType = sdtDomain then
    ...
    Result := SQLKeyWord(SQLDataTypeNames[DataType], Options);
  if (Len > 0) then
...
//    Result := Result + SQLKeyWord(' ON DELETE ' + Codes[OnDelete], Options);
  if OnUpdate <> fkaNone then
    Result := Result + SQLKeyWord(' ON UPDATE ' +
                                      ForeignKeyActionNames[OnUpdate], Options);
  if OnDelete <> fkaNone then
    Result := Result + SQLKeyWord(' ON DELETE ' +
                                      ForeignKeyActionNames[OnDelete], Options);
end;
