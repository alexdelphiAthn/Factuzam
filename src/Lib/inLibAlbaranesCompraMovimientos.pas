...
{    que es el unico que toca stock real (fza_articulos_stockactual). La       }
{    reversion borra los movimientos por (TIPO_DOC='AC' + SERIE + NUMERO)      }
{    y recalcula PMP + stock de los SKUs afectados via                         }
{    SP_RECALCULAR_PMP_LOTE_ALMACEN.                                           }
{                                                                              }
{    Fuente de datos para los movimientos:                                     }
{      - fza_albaranes_compra_celdas si la linea tiene celdas con              }
{        cantidad > 0 (caso edicion manual con tallas).                        }
{      - fza_albaranes_compra_lineas en caso contrario, una entrada por        }
{        linea con CANTIDAD_ALBCLIN (caso materializacion de sesion).          }
{                                                                              }
...
begin
  ...
  try
    ...
    qSrc.SQL.Text :=
      ...
      '       CASE WHEN IFNULL(A.ESIVA_RECARGO_COMPRAS_ALBC, ''N'') = ''S'' ' +
      ...
      '                  ELSE 0 END) / 100) ' +
      '            ELSE L.PRECIO_COMPRA_SIVA_ARTICULO_ALBCLIN END AS PRECIO, ' +
    ...
    begin
      ...
      spIns.ParamByName('p_TIPO_DOC_MOV').AsString        := 'AC';
      ...
      spIns.ParamByName('p_TIPO_MOVIMIENTO_MOV').AsString := 'E';
