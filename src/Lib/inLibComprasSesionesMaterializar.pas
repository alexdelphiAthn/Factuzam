...
begin
  ...
  try
    ...
    q.SQL.Text :=
      ...
      '              IFNULL(SUM(CASE WHEN IFNULL(H.ESIVA_EXENTO_INTRACOMUNITARIO_PEDC, ''N'') <> ''S'' AND IFNULL(H.ESIVA_RECARGO_COMPRAS_PEDC, ''N'') = ''S'' AND L.TIPO_IVA_ARTICULO_PEDCLIN = ''N'' THEN L.TOTAL_PEDCLIN * IFNULL(V.PORCENTAJE_NORMAL_RE_IVA, 0) / 100 ELSE 0 END), 0) AS RE_N, ' +
      '              IFNULL(SUM(CASE WHEN IFNULL(H.ESIVA_EXENTO_INTRACOMUNITARIO_PEDC, ''N'') <> ''S'' AND IFNULL(H.ESIVA_RECARGO_COMPRAS_PEDC, ''N'') = ''S'' AND L.TIPO_IVA_ARTICULO_PEDCLIN = ''R'' THEN L.TOTAL_PEDCLIN * IFNULL(V.PORCENTAJE_REDUCIDO_RE_IVA, 0) / 100 ELSE 0 END), 0) AS RE_R, ' +
      '              IFNULL(SUM(CASE WHEN IFNULL(H.ESIVA_EXENTO_INTRACOMUNITARIO_PEDC, ''N'') <> ''S'' AND IFNULL(H.ESIVA_RECARGO_COMPRAS_PEDC, ''N'') = ''S'' AND L.TIPO_IVA_ARTICULO_PEDCLIN = ''S'' THEN L.TOTAL_PEDCLIN * IFNULL(V.PORCENTAJE_SUPERREDUCIDO_RE_IVA, 0) / 100 ELSE 0 END), 0) AS RE_S, ' +
      '              IFNULL(SUM(CASE WHEN IFNULL(H.ESIVA_EXENTO_INTRACOMUNITARIO_PEDC, ''N'') <> ''S'' AND IFNULL(H.ESIVA_RECARGO_COMPRAS_PEDC, ''N'') = ''S'' AND L.TIPO_IVA_ARTICULO_PEDCLIN = ''E'' THEN L.TOTAL_PEDCLIN * IFNULL(V.PORCENTAJE_EXENTO_RE_IVA, 0) / 100 ELSE 0 END), 0) AS RE_E, ' +
      '              COUNT(*) AS NLIN ' +
      ...
      '       C.TOTAL_LIQUIDO_PEDC = T.BASE * GREATEST(0, 1 - IFNULL(C.PORCENTAJE_DTO_COMERCIAL_PEDC, 0) / 100) + (T.IVA_N + T.IVA_R + T.IVA_S + T.IVA_E + T.RE_N + T.RE_R + T.RE_S + T.RE_E) * GREATEST(0, 1 - IFNULL(C.PORCENTAJE_DTO_COMERCIAL_PEDC, 0) / 100) - T.BASE * GREATEST(0, 1 - IFNULL(C.PORCENTAJE_DTO_COMERCIAL_PEDC, 0) / 100) * IFNULL(C.PORCENTAJE_RETENCION_PEDC, 0) / 100 - T.BASE * GREATEST(0, 1 - IFNULL(C.PORCENTAJE_DTO_COMERCIAL_PEDC, 0) / 100) * IFNULL(C.PORCENTAJE_DTO_FINANCIERO_PEDC, 0) / 100, ' +
