...
{                                                                              }
{  Descripción:                                                                }
{    Adaptador UniDAC de las operaciones de pedidos de compra. Conserva        }
{    el SQL y la gestión transaccional extraídos de inLibPedidosCompra e       }
{    implementa IPedidosCompra.                                                }
{******************************************************************************}
...
function EjecutarRecepcionPedidoCompraInterno(AConn: TUniConnection;
  const AParametros: TParametrosRecepcionPedidoCompra;
  out AResultado: TResultadoRecepcionPedidoCompra): Boolean; forward;
procedure GenerarPdteRecibirDesdePedidoInterno(AConn: TUniConnection;
  const ASeriePedc, ANumPedc, AUsuario: string); forward;
procedure BorrarPdteRecibirDesdePedidoInterno(AConn: TUniConnection;
  const ASeriePedc, ANumPedc, ALinea: string); forward;
function CrearAlbaranDesdePedidoInterno(AConn: TUniConnection;
  const ASeriePedc, ANumPedc, ACodigoAlm, ASerieAlbc,
    AUsuario, ARefPrv: string;
  AFechaRecepcion: TDateTime;
  AIdPvTemporada: Integer;
  out ANumAlbc, AMensaje: string): Boolean; forward;
...
function CalcularPendienteTotalInterno(AConn: TUniConnection;
  const ASeriePedc, ANumPedc: string): Double; forward;
function IncorporarAlbaranDesdePedidoInterno(AConn: TUniConnection;
  const ASeriePedc, ANumPedc, ACodigoAlm,
    ASerieAlbcDestino, ANumAlbcDestino, AUsuario: string;
  AIdPvTemporada: Integer;
  out AMensaje: string): Boolean; forward;
...
// ===========================================================================
//   TPedidosCompraUniDAC - adaptador del contrato
// ===========================================================================
constructor TPedidosCompraUniDAC.Create(
  AConexion: TUniConnection);
begin
...
begin
  Result := CrearAlbaranDesdePedidoInterno(
    ...
    ASerieAlbc, AUsuario, ARefPrv, AFechaRecepcion,
    AIdPvTemporada, ANumAlbc, AMensaje);
end;
...
begin
  Result := CrearAlbaranDesdePedidoConCantidadesInterno(
    FConexion, ASeriePedc, ANumPedc, ACodigoAlm,
    ASerieAlbc, AUsuario, ARefPrv, AFechaRecepcion,
    AIdPvTemporada, ACeldas, ANumAlbc, AMensaje);
end;
...
function TPedidosCompraUniDAC.EjecutarRecepcionPedidoCompra(
  ...
  out AResultado: TResultadoRecepcionPedidoCompra): Boolean;
begin
  Result := EjecutarRecepcionPedidoCompraInterno(
    FConexion, AParametros, AResultado);
end;
