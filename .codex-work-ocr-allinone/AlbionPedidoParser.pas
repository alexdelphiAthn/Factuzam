...
function PosTexto(const ASubcadena, ATexto: string): Integer;
begin
  Result := Pos(UpperCase(ASubcadena), UpperCase(ATexto));
end;
...
begin
  Inicio := PosTexto(AEtiqueta, ATexto);
  if Inicio = 0 then
    Exit('');
...
var
  ...
  Bloque: string;
begin
  Inicio := PosTexto(CProveedor, ATexto);
  if Inicio = 0 then
    Exit('');
  Inc(Inicio, Length(CProveedor));
  Fin := PosTexto('CIF:', ATexto);
...
begin
  Result := Trim(ATexto);
  PosPrice := PosTexto('PRICE WS', Result);
  PosPrecio := PosTexto('PRECIO WS', Result);
  Corte := 0;
  if PosPrice > 0 then
    Corte := PosPrice;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if (PosTexto('PRICE WS', Parte) > 0) or
           (PosTexto('PRECIO WS', Parte) > 0) then
          FilaPrecio := F;
...
begin
  ...
  Referencia := ValorDebajo(ATablas, ['Order N', 'Pedido']);
...
begin
  ...
  try
    ...
    if not ContainsText(CadenaJson(Analisis, 'content'), CProveedor) then
      raise EAlbionPedidoParser.Create(
        'El documento no pertenece al proveedor Albion esperado.');
