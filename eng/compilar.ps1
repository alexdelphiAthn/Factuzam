...
$plataformas = if ($Plataforma -eq 'Ambas') {
  @('Win32', 'Win64')
}
else {
  @($Plataforma)
...
if ($errores.Count -gt 0) {
  Write-Output 'No se puede compilar Factuzam:'
...
Write-Output (
  'Dependencias de compilación: OK. Plataformas: ' +
  ($plataformas -join ', ') + '.')
if ($SoloValidar) {
  exit 0
}
...
foreach ($destino in $plataformas) {
  $salidaDcu = Join-Path $raiz (
    "build\reproducible\dcu\$destino\$Configuracion")
  $salidaExe = Join-Path $raiz (
    "build\reproducible\bin\$destino\$Configuracion")
  [void](New-Item -ItemType Directory -Path $salidaDcu -Force)
  [void](New-Item -ItemType Directory -Path $salidaExe -Force)
  Write-Output "Compilando fzam $Configuracion $destino..."
  $argumentos = @(
    '"' + $proyecto + '"',
    '/t:Build',
  ...
  Write-Output "Compilación correcta: $Configuracion $destino."
}
